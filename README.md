...
3. The workflow in `.github/workflows/deploy-pages.yml` builds and publishes the frontend automatically on every push to `main`.

GitHub Pages hosts only the Vite frontend. Without a separately deployed backend,
the app uses its labelled demo data fallback. To connect live market data, deploy
`server/` separately and add a GitHub Actions variable named `VITE_API_BASE_URL`
with the public URL of that backend. Add `VITE_SUPABASE_URL` and
`VITE_SUPABASE_ANON_KEY` as repository variables if Supabase-backed features are
needed. Never add the backend service-role key to GitHub Actions variables used by
the frontend build.
